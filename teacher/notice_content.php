...
<?php
  ...
  $sqlstr="select * from user where User_no =".$_SESSION['user_no'];
	...
	$row = mysql_fetch_array($result, MYSQL_BOTH);//1//
...
$sqlstr="select * from notice where Notice_ID=".$_GET["id"];
	$result = mysql_query($sqlstr,$link);
...
<script> 
    ...
    <div class="right">
      <form action="notice_chg.php?id=<?=$notice_id?>" method="post"><img src="../images/img_8.gif" style="float:left"/>
<div class="news">公告题目：<textarea name="Notice_title" id="Notice_title" rows="1" cols="80" ><?=$notice_title?></textarea></div>
      <div class="news">公告内容：<textarea name="Notice_content" id="Notice_content" rows="10" cols="80"><?=$notice_content?></textarea>
    <input  style="float:left" name="input" type="image" src="../images/img_67.gif"></form>
    <input  style="float:left" name="input" type="image" src="../images/img_68.gif" onclick="javascript:window.location.href='notice_chg.php?id=<?=$notice_id?>&type=2'">
