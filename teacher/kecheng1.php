...
//要创建的两个文件 
 $biaoti = $_POST['biaoti'];
$TxtFileName ='../keben/wendang/'.$_POST['xuhao'].' '.$biaoti.'.txt';//problem//
 $TxtFileName=iconv("UTF-8","gb2312",$TxtFileName);
...
if( ($TxtRes=fopen ($TxtFileName,"w+")) === FALSE){     
    ...
    $StrConents = $_POST['inputText'];//要写进文件的内容
...
fclose ($TxtRes); //关闭指针 
$TxtFileName1  ='../student/'.$_POST['xuhao'].'.php';
$TxtFileName2  ='../teacher/'.$_POST['xuhao'].'.php';
if( ($TxtRes1=fopen ($TxtFileName1,"w+")) === FALSE){    
  
    exit(); 
  
} 
fclose ($TxtRes1); //关闭指针
if( ($TxtRes2=fopen ($TxtFileName2,"w+")) === FALSE){    
  
    exit(); 
  
} 
fclose ($TxtRes2); //关闭指针
$file = '../teacher/kechengmuban.php';
$newfile = '../student/'.$_POST['xuhao'].'.php'; # 这个文件父文件夹必须能写
$result = copy($file, $newfile);


$file= '../teacher/tea_muban.php';
$newfile = '../teacher/'.$_POST['xuhao'].'.php'; # 这个文件父文件夹必须能写
$result = copy($file, $newfile);
echo  "<script>alert('添加课程内容成功')</script>";
?>
