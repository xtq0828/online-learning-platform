...
$sqlstr="select *  from student_papers as sp left join user  on user.User_no = sp.User_no   where sp.papers_id='".$_GET['pid']."'";
...
//查询学生最高成绩和最低成绩
$sqlmax="select max(score) as gao,min(score) as di from student_papers    where papers_id='".$_GET['pid']."'";
  ...
  <div class="main">
    ...
    <div class="right">
      ...
      <div class="news">
        ...
        <span  style="display:block; width:400px;">学生作业情况:</span><span><a style="font-weight:blod; color:red;"   href="newwork.php" >新建作业</a></span><a style="font-weight:blod; color:red;" href="teachworklist.php">查看作业列表</a></div>
       
         <div class="news" style="height:500px;">
     
	      <table width="100%"  border="1" cellspacing="0" cellpadding="0" style="text-align:center">
	     		<tr><td width="15%"  style="text-align:center"  height="50px;" >学生学号</td><td width="15%"  >学生姓名</td><td width="70%" >学生成绩</td></tr>
	      		
	      		<?php if($rows){  foreach ($rows as $student){ ?>
		      		<tr><td height="30px;"><?php echo $student['User_no']; ?></td><td><?php echo $student['User_realname']; ?></td>
		      				
		      				<td><span <?php if($rowmax['gao']==$student['score']){ ?>style="font-weight:bold; color:red;" <?php  }elseif($rowmax['di']==$student['score']){ ?>style="color:red;"  <?php } ?>><?php echo $student['score']; ?></span></td>
