          ...
          <tr>
            <td colspan="2">
             ...
             <input name="input" type="image" src="../images/img_63.gif" onclick="javascript:window.location.href='add_stu.php'"/>
              <input name="input" type="image" src="../images/img_69.gif" onclick="javascript:window.location.href='#'">
...
<script> 
    ...
    <div class="right">
      ...
      <div class="news">
        ...
        <tr>
  		<td width="20%" height="18"><a style="font-weight:bold">学生学号</a></td>
        <td width="20%" align="center" style="font-weight:bold">学生姓名</td>
        <td width="20%" align="center" style="font-weight:bold">最近一次登录月份</td>
		<td width="20%" align="center" style="font-weight:bold">本月登陆次数</td>
        <td width="20%" align="center" style="font-weight:bold">本月平均学习时间（分钟）</td>
        </tr>
        <?php  
		$sqlstr="select * from stu_model,user where stu_model.Stu_no=user.User_no Order by Stu_no ASC";
	    $result = mysql_query($sqlstr,$link);
    	while($row=mysql_fetch_row($result)){ ?>
	    <tr>
  		<td width="20%" height="18"><?php echo $row[1]; ?></td>
        <td width="20%" align="center"><?php echo $row[10]; ?></td>
        <td width="20%" align="center"><?php if(strlen($row[7])==0){echo "本月未登录过学习系统";}else{echo $row[7];} ?></td>
		<td width="20%" align="center"><?php if($row[2]==0){echo "/";}else{echo $row[2];} ?></td>
        <td width="20%" align="center"><?php if($row[6]==0){echo "/";}else{echo $row[6];} ?></td>	
        </tr>
	    <?php } 
	    mysql_free_result($result);
        mysql_close($link);?>  
        </table></div>
        <div><input  name="input" type="image" src="../images/img_66.gif" onclick="javascript:window.location.href='tea_index.php'"></div>
      </div>
